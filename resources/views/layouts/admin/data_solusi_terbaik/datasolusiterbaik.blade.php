...
<head>
  ...
  <title>Mental Care - Data Solusi Terbaik</title>
...
<body>
  ...
  <main id="main" class="main">
    ...
    <h1>
      <div class="text-dark"> Data Solusi Terbaik </div>
    </h1>
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="text-dark breadcrumb-item active" aria-current="page">Data Solusi Terbaik</li>
      </ol>
    </nav>
    </div><!-- End Page Title -->
      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="d-grid gap-2 d-md-flex align-content-md-end">
            <a href="{{ route('layouts.admin.data_solusi_terbaik.datasolusiterbaik_create') }}" class="btn btn-success" type="button">
              <i class=""></i>Tambah</a>
          </div>
          <br>
          <div class="card ">
            <div class="card-body">
              ...
              <table class="table table-striped">
                <thead>
                  <tr>
                    <th scope="col">No</th>
                    <th scope="col">Kategori Depresi</th>
                    <th scope="col">Solusi Terbaik</th>
                    <th scope="col">Opsi</th>
                  </tr>
                </thead>
                <tbody>
                <tbody>
                  @if(!empty($datasolusiterbaik) && $datasolusiterbaik->count())
                  @foreach($datasolusiterbaik as $datasolusiterbaiks)
                  <tr>
                    <td scope="row">{{ $loop->iteration }}</td>
                    <td>{{ $datasolusiterbaiks->kategori_depresi }}</td>
                    <td>{{ $datasolusiterbaiks->solusi_terbaik}}</td>
                    <td>
                      <div class="d-flex">
                        <div class="me-2">
                          <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#editModal{{$datasolusiterbaiks->id}}">
                            Edit
                          </button>
                        </div>
                        <div class="me-2">
                          <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{$datasolusiterbaiks->id}}">
                            Hapus
                          </button>
                        </div>

                        <!-- Edit Modal -->
                        <div class="modal fade" id="editModal{{$datasolusiterbaiks->id}}" tabindex="-1" aria-labelledby="editModalLabel{{$datasolusiterbaiks->id}}" aria-hidden="true">
                          <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h1 class="modal-title fs-5" id="editModalLabel{{$datasolusiterbaiks->id}}">Edit Data Solusi Terbaik</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                              </div>
                              <div class="modal-body">
                                <form action="{{ route('layouts.admin.data_solusi_terbaik.datasolusiterbaik_update', $datasolusiterbaiks->id) }}" method="POST">
                                  @csrf
                                  @method('PUT')
                                  <!-- Form fields go here -->
                                  <div class="form-group">
                                    <label for="kategori_depresi">Kategori Depresi</label>
                                    <select class="form-control" id="kategori_depresi" name="kategori_depresi">
                                      <option value="depresi normal">Depresi Normal</option>
                                  ...
                                  <div class="form-group">
                                    <label for="solusi_terbaik">Solusi Terbaik</label>
                                    <textarea class="form-control" id="solusi_terbaik" rows="3" name="solusi_terbaik">{{$datasolusiterbaiks->solusi_terbaik}}</textarea>
                                  </div>
                              </div>
                        ...
                        <!-- Delete Modal -->
                        <div class="modal fade" id="deleteModal{{$datasolusiterbaiks->id}}" tabindex="-1" aria-labelledby="deleteModalLabel{{$datasolusiterbaiks->id}}" aria-hidden="true">
                          <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h1 class="modal-title fs-5" id="deleteModalLabel{{$datasolusiterbaiks->id}}">Hapus Data Solusi Terbaik</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                              </div>
                              <div class="modal-body">
                              ...
                              <div class="modal-footer">
                                <form action="{{ route('layouts.admin.data_solusi_terbaik.datasolusiterbaik_destroy', $datasolusiterbaiks->id) }}" method="post">
                                  @csrf
                                  @method('DELETE')
