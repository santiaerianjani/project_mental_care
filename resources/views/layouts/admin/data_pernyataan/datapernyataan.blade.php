...
<head>
    ...
    <title>Mental Care - Data Pernyataan</title>
...
<body>
    ...
    <main id="main" class="main">
        ...
        <h1>
            <div class="text-dark"> Data Pernyataan </div>
        </h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="text-dark breadcrumb-item active" aria-current="page">Data Pernyataan</li>
            </ol>
        </nav>
        </div><!-- End Page Title -->
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="d-grid gap-2 d-md-flex align-content-md-end">
                        <a href="{{ route('layouts.admin.data_pernyataan.datapernyataan_create') }}" class="btn btn-success" type="button">
                            <i class=""></i>Tambah Pernyataan</a>
                    </div>
                    <br>
                    <div class="card ">
                        <div class="card-body">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                <tbody>
                                    <!-- @if(!empty($datapernyataan) && $datapernyataan->count()) -->
                                    @foreach($datapernyataan as $datapernyataans)
                                    <tr>
                                        <td scope="row">{{ $loop->iteration }}</td>
                                        <td>{{ $datapernyataans->pernyataan }}</td>
                                        @php
                                        $jawaban = App\Models\DataJawaban::where('pernyataan_id',$datapernyataans->id)->get();
                                        @endphp
                                        <td>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <div class="me-2">
                                                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#tambahModal{{$datapernyataans->id}}">
                                                        Tambah Opsi
                                                    </button>
                                                </div>
                                                <div class="me-2">
                                                    <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{$datapernyataans->id}}">
                                                        Hapus
                                                    </button>
                                                </div>

                                                <!-- Tambah Opsi Modal -->
                                                <div class="modal fade" id="tambahModal{{$datapernyataans->id}}" tabindex="-1" aria-labelledby="tambahModalLabel{{$datapernyataans->id}}" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h1 class="modal-title fs-5" id="tambahModalLabel{{$datapernyataans->id}}">Tambah Jawaban</h1>
                                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <div class="modal-body">
                                                                <form action="{{ route('layouts.admin.data_opsi', $datapernyataans->id)}}" method="POST">
                                                                    @csrf
                                                                    <div class="form-group">
                                                                        <label for="jawaban">Jawaban</label>
                                                                        <textarea class="form-control" id="jawaban" rows="3" name="jawaban"></textarea>
                                                                    </div>
                                                                    <div class="form-group">
                                                                        <label for="point">Point</label>
                                                                        <input type="number" class="form-control" id="point" rows="3" name="point"></input>
                                                                    </div>
                                                            </div>
                                                ...
                                                <!-- Delete Modal -->
                                                <div class="modal fade" id="deleteModal{{$datapernyataans->id}}" tabindex="-1" aria-labelledby="deleteModalLabel{{$datapernyataans->id}}" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h1 class="modal-title fs-5" id="deleteModalLabel{{$datapernyataans->id}}">Hapus Data Pernyataan</h1>
                                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <div class="modal-body">
                                                            ...
                                                            <div class="modal-footer">
                                                                <form action="{{ route('layouts.admin.data_pernyataan.datapernyataan_destroy', $datapernyataans->id) }}" method="post">
                                                                    @csrf
                                                                    @method('DELETE')
