...
<head>
  ...
  <title>Mental Care - Hasil Solusi Terbaik</title>
...
<body>
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <!-- Tanggap -->
      <li class="nav-item">
        <a class="nav-link active" data-bs-toggle="collapse" data-bs-target="#tanggap-nav" aria-expanded="false">
        ...
        <ul id="tanggap-nav" class="nav-content collapse" data-bs-parent="#sidebar-nav">
          <li>
            <a class="nav-link active" href="{{ route('layouts.user.hasil_solusi_terbaik.hasilsolusiterbaik') }}">
          ...
          <div class="card ">
            <div class="card-body">
              ...
              <table class="table table-striped">
                ...
                <tbody>
                  @foreach ($hasilsolusiterbaiks as $index => $hasilsolusiterbaik)
                  <tr>
                    <td scope="row">{{ $index + 1 }}</td>
                    <td>{{ $hasilsolusiterbaik->id }}</td>
                    <td>{{ $hasilsolusiterbaik->datasolusiterbaik->kategori_depresi }}</td>
                    <td>
                      <div class="d-flex">
                        <div class="mr-2">
                          <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#viewModal{{ $hasilsolusiterbaik->id }}">
                            Lihat Solusi
                          </button>
                        </div>
                        <!-- View Modal -->
                        <div class="modal fade" id="viewModal{{ $hasilsolusiterbaik->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h1 class="modal-title fs-5" id="viewModalLabel"></h1>
                                <h4 class="font-weight-bold">Solusi Terbaik Untuk Anda</h4>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                              </div>
                              <div class="modal-body">
                                @php
                                $data = App\Models\HasilSolusiTerbaik::find($hasilsolusiterbaik->id);
                                @endphp
                                <p>{{ $data->datasolusiterbaik->solusi_terbaik }}</p>
                              </div>
